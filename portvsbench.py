...
if st.button('Next'):
    if assets_input == ['BHARTIARTL.NS', 'HDFCBANK.NS','HINDUNILVR.NS','ICICIBANK.NS','INFY.NS','ITC.NS','LT.NS','RELIANCE.NS','SBIN.NS','TCS.NS']:
        ...
        st.markdown("**Pie Chart of Sector Weights Allocated Benchmark:**")
        ...
        st.markdown("**Pie Chart of Stock Weights Portfolio:**")
        ...
        # POINT (a)
        #st.markdown('**Line chart of Portfolio against the Benchmark (Rebased to 100 for initial date)**')
        ...
        st.write('Portfolio values of Attribution Report:')
        st.write(stock_closing_prices_hdfc)
        ...
        st.markdown('<p style="font-size:20px;"><b>Top 10 relative weight, Bottom 10 relative weight</b></p>', unsafe_allow_html=True)
        ...
        df_bench = pd.DataFrame(sorted_contribution_bench, columns=['Company', 'Contribution to return(%)'])
        df_bench = df_bench.sort_values(by='Contribution to return(%)', ascending=False)
        col7, col8 = st.columns(2)
        with col7:
            st.markdown('**Contribution to return - Portfolio, Top 10**')
            st.write(df_port.set_index('Company'))
        with col8:
            st.markdown('**Contribution to return - Benchmark, Bottom 10**')
        ...
        st.markdown('<p style="font-size:20px;"><b>Penny Stocks</b></p>', unsafe_allow_html=True)
        st.text('There are no penny stocks included in the portfolio as all of the listed stocks fall under HDFCNIFTY50')

        #P/E ratio
        st.markdown('<p style="font-size:20px;"><b>P/E</b></p>', unsafe_allow_html=True)
        PE_frame=pd.read_csv('MarketCap.csv')
        PE_frame['Earnings'] = PE_frame.apply(lambda row: (row['Market Cap(cr)']/row['PE']), axis=1)
        Total_Weighted_Mcap = ((PE_frame['Weightage(%)']/100) * PE_frame['Market Cap(cr)']).sum()
...
if st.button('Rebalancing'):
    ...
    portfolio_valuesss = stock_closing_prices_hdfc.apply(lambda row: sum(row[stock] * optimal_stocks_hdfc[stock] for stock in optimal_stocks_hdfc), axis=1)
    stock_closing_prices_hdfc['Portfolio Value'] = portfolio_valuesss
    dates_to_extract = ['2023-02-28', '2023-05-31', '2023-08-31', '2023-11-30', '2024-01-30']

    # Convert the 'Date' column to datetime format if it isn't already
    stock_closing_prices_hdfc['Date'] = pd.to_datetime(stock_closing_prices_hdfc['Date'])

    # Filter the DataFrame by the specified dates
    filtered_data = stock_closing_prices_hdfc[stock_closing_prices_hdfc['Date'].isin(pd.to_datetime(dates_to_extract))]
    ...
    fig_rebalancing.add_trace(go.Scatter(x=df_fta['Date'], y=df_fta['Value']/10000000, mode='lines+markers', name='Portfolio Value With Rebalancing', line=dict(color='red'), showlegend=True))
    fig_rebalancing.add_trace(go.Scatter(x=filtered_data['Date'], y=filtered_data['Portfolio Value']/10000000, mode='lines+markers', name='Portfolio Value Without Rebalancing', line=dict(color='blue'), showlegend=True))

    fig_rebalancing.update_layout(title='Rebalanced Portfolio Values Over Time',
        xaxis_title='Date', 
        yaxis_title='Portfolio Value(cr)',
        autosize=False, 
        width=1000, 
        height=600,
        yaxis_range=[0.15,0.22])
